...
public class Task1A {
    ...
    public static class Node {
        ...
        public Node(int maxInRange, int maxIndex) {
    ...
    public static Node combine(Node leftChild, Node rightChild) {
        if (leftChild.maxInRange > rightChild.maxInRange) {
            return new Node(leftChild.maxInRange, leftChild.maxIndex);
        } else {
            return new Node(rightChild.maxInRange, rightChild.maxIndex);
        }
    }

    public static void build(Node[] tree, int[] a, int v, int l, int r) {
        if (l == r) {
            tree[v] = new Node(a[l], l);
        } else {
            int m = (l + r) / 2;
    ...
    public static void main(String[] args) {
        ...
        while (k > 0) {
            ...
            ans.append(res.maxInRange).append(" ").append(res.maxIndex + 1).append('\n');
            k--;
        }
